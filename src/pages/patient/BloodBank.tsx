...
const BloodBank = () => {
  ...
  const broadcast = async () => {
    ...
    // fake donors
    const fakeDonors = [
      { user_id: 1, full_name: "Rahul", blood_type: "O+", lat: 19.048, lng: 73.070 },
      { user_id: 2, full_name: "Amit", blood_type: "O+", lat: 19.045, lng: 73.068 },
      { user_id: 3, full_name: "Sahil", blood_type: "O+", lat: 19.050, lng: 73.066 },
      { user_id: 4, full_name: "Neha", blood_type: "O+", lat: 19.046, lng: 73.072 },
    ];

    const nearby = fakeDonors.map((d) => ({
      ...d,
    ...
    toast.success(`Broadcast sent to ${nearby.length} nearby donors`);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Broadcast to matched donors in nearby area
          </p>
        </div>
      </div>
        ...
        <Card className="overflow-hidden p-0">
          ...
          <div className="h-[320px]">
            <MapContainer center={[coords.lat, coords.lng]} zoom={14} className="h-full w-full">
              ...
              {/* SMALLER CIRCLE */}
              <Circle center={[coords.lat, coords.lng]} radius={1500} pathOptions={{ color: "red" }} />
      ...
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {requests.map((r, i) => {
            const dist = (Math.random() * 3).toFixed(1);
              ...
              <Card key={i} className="p-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    <MapPin className="inline h-3 w-3" /> {dist} km
                  </span>
                </div>

                <div className="mt-2 font-semibold">
                  {r.units_needed} unit(s) · {r.hospital || "Apollo Hospital, Kharghar"}
                </div>
              </Card>
            );
          })}
